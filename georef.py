...
class Georef:
    ...
    def clean_data(self,data):
        df=pd.DataFrame(data,columns=['Profile','Index','X','Y','Z','Alpha','Beta','Gama'])
    ...
    def rot(self,angles):
        ...
        m12=np.sin(x_rot)*np.sin(y_rot)*np.cos(z_rot)+np.cos(x_rot)*np.sin(z_rot)
        m13=-np.cos(x_rot)*np.sin(y_rot)*np.cos(z_rot)+np.sin(x_rot)*np.sin(z_rot)
        m21=-np.cos(y_rot)*np.sin(z_rot)
        m22=-np.sin(x_rot)*np.sin(y_rot)*np.sin(z_rot)+np.cos(x_rot)*np.cos(z_rot)
        m23=np.cos(x_rot)*np.sin(y_rot)*np.sin(z_rot)+np.sin(x_rot)*np.cos(z_rot)
        m31=np.sin(y_rot)
    ...
    def save_georef_data(self,path:str):
        ...
        """
        try:
            pd.DataFrame(self.georef_data,columns=['X','Y','Z']).to_csv(path,index=False)
